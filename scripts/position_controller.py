...
class Edrone():
    ...
    def __init__(self):
        ...
        self.throttle_setpoint = 1500

        self.altitude_setpoint  = 3
        self.latitude_setpoint  = 19.0000451704 
        self.longitude_setpoint = 72.0
        ...
        self.sample_time = 30.0  # in seconds

        # Publishing /edrone/pwm, /roll_error, /pitch_error, /yaw_error
        self.pwm_pub = rospy.Publisher('/edrone/pwm', prop_speed, queue_size=1)
        # ------------------------Add other ROS Publishers here-----------------------------------------------------
        
        self.pub_roll_error  = rospy.Publisher('/roll_error', Float32, queue_size=10)
        self.pub_pitch_error = rospy.Publisher('/pitch_error', Float32, queue_size=10)
        self.pub_yaw_error   = rospy.Publisher('/yaw_error', Float32, queue_size=10)

        self.pub_Altitude_error  = rospy.Publisher('/Altitude_error', Float32, queue_size=10)
        self.pub_latitude_error  = rospy.Publisher('/latitude_error', Float32, queue_size=10)
        self.pub_longitude_error = rospy.Publisher('/longitude_error', Float32, queue_size=10)

        self.pub_zero_error = rospy.Publisher('/zero_error', Float32, queue_size=10)
    ...
    """def drone_command_callback(self, msg):
        ...
        self.throttle_setpoint = msg.rcThrottle"""
    ...
    def pid(self):
        ...
        #   5. Calculate the pid output required for each axis. For eg: calcuate self.out_roll, self.out_pitch, etc.       #   6. Use this computed output value in the equations to compute the pwm for each propeller. LOOK OUT FOR SIGN (+ or -). EXPERIMENT AND FIND THE CORRECT SIGN
        ...
        if self.altitude // 1 != 3 and self.latitude < 19.00003:
            ...
            self.throttle_value = self.throttle_setpoint * 1.023 - 1023
            ...
            self.error[3] = self.altitude_setpoint - self.altitude 
            #
            self.present_time = time.time()
            self.time_change  = self.present_time - self.last_time

        else:

            if 19.00004110 < self.latitude < 19.000046:

                self.error[5] = self.longitude - self.longitude_setpoint
                #
                self.present_time = time.time()
                self.time_change  = self.present_time - self.last_time
                #
                self.error_sum[5] += self.error[5] * self.time_change
                self.error_diff[5] = (self.error[5] - self.prev_error[5]) / self.time_change
                #
                self.pid_value[5] = self.Kp[5]*self.error[5] + self.Ki[5]*self.error_sum[5] + self.Kd[5]*self.error_diff[5]
                #
                self.new_setpoint_cmd_0 = self.setpoint_cmd[0] + self.pid_value[5]

                # Convertng the range from 1000 to 2000 in the range of -10 degree to 10 degree for roll axis
                self.setpoint_euler[0] = self.new_setpoint_cmd_0 * 0.02 - 30
                self.setpoint_euler[1] = self.setpoint_cmd[1] * 0.02 - 30
                self.setpoint_euler[2] = self.setpoint_cmd[2] * 0.02 - 30
                ...
                self.error[1] = self.setpoint_euler[1] - self.drone_orientation_euler[1]
                self.error[2] = self.setpoint_euler[2] - self.drone_orientation_euler[2]
                self.error[3] = 0.31 - self.altitude 
                #
                
                if self.error[3] >= - 0.13:

                    self.throttle_value = 0

            else:

                self.error[4] = self.latitude_setpoint - self.latitude
                self.error[5] = self.longitude - self.longitude_setpoint 
                #
                self.present_time = time.time()
                self.time_change  = self.present_time - self.last_time
                #
                self.error_sum[4] += self.error[4] * self.time_change
                self.error_diff[4] = (self.error[4] - self.prev_error[4]) / self.time_change
                self.error_sum[5] += self.error[5] * self.time_change
                self.error_diff[5] = (self.error[5] - self.prev_error[5]) / self.time_change
                #
                self.pid_value[4] = self.Kp[4]*self.error[4] + self.Ki[4]*self.error_sum[4] + self.Kd[4]*self.error_diff[4]
                self.pid_value[5] = self.Kp[5]*self.error[5] + self.Ki[5]*self.error_sum[5] + self.Kd[5]*self.error_diff[5]
                #
                self.new_setpoint_cmd_0 = self.setpoint_cmd[0] + self.pid_value[5]
                self.new_setpoint_cmd_1 = self.setpoint_cmd[1] + self.pid_value[4]

                # Convertng the range from 1000 to 2000 in the range of -10 degree to 10 degree for roll axis
                self.setpoint_euler[0] = self.new_setpoint_cmd_0 * 0.02 - 30
                self.setpoint_euler[1] = self.new_setpoint_cmd_1 * 0.02 - 30
                self.setpoint_euler[2] = self.setpoint_cmd[2] * 0.02 - 30
        ...
        #
        self.error_sum[0] += self.error[0] * self.time_change
        self.error_diff[0] = (self.error[0] - self.prev_error[0]) / self.time_change
        #
        self.error_sum[1] += self.error[1] * self.time_change
        self.error_diff[1] = (self.error[1] - self.prev_error[1]) / self.time_change
        #
        self.error_sum[2] += self.error[2] * self.time_change
        self.error_diff[2] = (self.error[2] - self.prev_error[2]) / self.time_change
        #
        self.error_sum[3] += self.error[3] * self.time_change
        self.error_diff[3] = (self.error[3] - self.prev_error[3]) / self.time_change
        #
        self.pid_value[0] = self.Kp[0]*self.error[0] + self.Ki[0]*self.error_sum[0] + self.Kd[0]*self.error_diff[0]
        self.pid_value[1] = self.Kp[1]*self.error[1] + self.Ki[1]*self.error_sum[1] + self.Kd[1]*self.error_diff[1]
        self.pid_value[2] = self.Kp[2]*self.error[2] + self.Ki[2]*self.error_sum[2] + self.Kd[2]*self.error_diff[2]
        self.pid_value[3] = self.Kp[3]*self.error[3] + self.Ki[3]*self.error_sum[3] + self.Kd[3]*self.error_diff[3]
        #
        #
        self.pwm_cmd.prop1 = self.throttle_value + self.pid_value[0]  - self.pid_value[1] - self.pid_value[2] + self.pid_value[3]
        self.pwm_cmd.prop2 = self.throttle_value - self.pid_value[0]  - self.pid_value[1] + self.pid_value[2] + self.pid_value[3]
        self.pwm_cmd.prop3 = self.throttle_value - self.pid_value[0]  + self.pid_value[1] - self.pid_value[2] + self.pid_value[3]
        self.pwm_cmd.prop4 = self.throttle_value + self.pid_value[0]  + self.pid_value[1] + self.pid_value[2] + self.pid_value[3]         
        #
        #
        ...
        #
        self.pub_roll_error.publish(self.error[0])  
        self.pub_pitch_error.publish(self.error[1])
        self.pub_yaw_error.publish(self.error[2])
        self.pub_Altitude_error.publish(self.error[3])
        self.pub_latitude_error.publish(self.error[4])
        self.pub_longitude_error.publish(self.error[5])
        self.pub_zero_error.publish(self.zero)
        #
        self.prev_error[0] = self.error[0]
        self.prev_error[1] = self.error[1]
        self.prev_error[2] = self.error[2]
        self.prev_error[3] = self.error[3]
        self.prev_error[4] = self.error[4]
        self.prev_error[5] = self.error[5]
        # 
        self.last_time = self.present_time
